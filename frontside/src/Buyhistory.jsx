    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const response = await fetch('http://localhost:3000/bid/history', options);
    ...
    return (
        <div>
            <div>
                ...
                <div>
                    <h1 className="title">Buy history</h1>
                    {products.rows ? (
...
function Listalldetails({ array }) {
    ...
    const handleClick = (productid) => {
        navigate('/buystatus', { state: { productid: productid } });
    };
        ...
        <section className="cards">
            {array.map((element, index) => (
                <div className="card">
                    ...
                    <button className="postButton" type="submit" onClick={() => handleClick(element.productid)}>view status</button>
